...
async function snapshot(loc:Location,stage:string){
  add(`--- ${stage} ---`);
  try{const s=await invoke<ConnState>('connection_state');add(`state=${s.state} connected=${s.connected} exit=${s.exit_country||'—'} public_ip=${s.public_ip||'—'}`)}catch(e){add(`connection_state error: ${String(e)}`)}
  const direct=(knownEndpoints[loc.host]||[])[0];
  if(direct){const route=await helper('route-explain',[direct]);add(`direct-endpoint route (${direct}):\n${route}`)}
  const dnsRoute=await helper('route-explain',[loc.host]);add(`hostname resolution/route (${loc.host}):\n${dnsRoute}`);
}
async function pullBackend(){
  try{
    const raw=await invoke<string>('connection_attempt_log');
    if(raw&&raw!==lastBackend){
      const delta=raw.startsWith(lastBackend)?raw.slice(lastBackend.length).replace(/^\n/,''):raw;
      lastBackend=raw;
      if(delta.trim())add(`backend:\n${delta}`);
    }
  }catch{}
}
async function collectAfter(loc:Location){await pullBackend();await snapshot(loc,'POST-ATTEMPT SNAPSHOT');const health=await helper('health');add(`health:\n${health}`);const live=await helper('full-live-test');add(`live verification:\n${live}`);add('--- END ATTEMPT ---');}
async function beginAttempt(){
  if(monitoring)return;
  monitoring=true;lines=[];lastBackend='';render();let locs:Location[]=[];
  try{locs=await invoke<Location[]>('list_locations')}catch(e){add(`Could not read location catalog: ${String(e)}`);monitoring=false;return}
  const loc=currentLocation(locs);if(!loc){monitoring=false;return}
  ...
  const started=Date.now();lastPhase='';
  const timer=window.setInterval(async()=>{
    await pullBackend();
    const phase=(document.querySelector('.hero-area h2')?.textContent||'').trim();
...
function build(){
  if(document.getElementById('milmit-connection-log-toggle'))return;
  const connect=document.querySelector<HTMLButtonElement>('.home-actions > .connect-btn, .home-actions > .disconnect-btn');if(!connect)return;
  ...
  panel=document.createElement('section');panel.id='milmit-connection-log-panel';panel.className='milmit-log-panel';panel.innerHTML='<div class="milmit-log-head"><div><b>Connection Log</b><small>Sanitized VPN diagnostics · direct endpoint + DNS view</small></div><div><button data-copy>Copy</button><button data-clear>Clear</button><button data-close>×</button></div></div><pre class="milmit-log-body">No connection attempt captured yet.</pre>';
  ...
  panel.querySelector<HTMLButtonElement>('[data-clear]')!.onclick=()=>{lines=[];lastBackend='';render()};
