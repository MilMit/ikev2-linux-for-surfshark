...
if [ ! -s /var/lib/milmit-surfshark/rules/ircidr.txt ] && [ -x /usr/lib/milmit-surfshark/rules-update.py ]; then
  /usr/lib/milmit-surfshark/rules-update.py update >/var/log/milmit-surfshark-rules-update.log 2>&1 || true
fi

if [ -x /usr/lib/milmit-surfshark/desktop-features.py ]; then
