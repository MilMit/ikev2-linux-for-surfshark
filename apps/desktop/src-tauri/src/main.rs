...
fn sanitize_backend(text: &str) -> String {
    ...
    for line in text.lines() {
        ...
        if l.contains("remote public key authentication") {
            local_auth = false;
        }
        if local_auth
            && (line.trim_start().starts_with("id:") || line.trim_start().starts_with("eap_id:"))
        {
            out.push(format!(
                "{}[redacted]",
        ...
        if l.contains("service_pass")
            || l.contains("service_user")
            || l.contains("password")
            || l.contains("secret")
            || l.contains("eap identity")
            || l.contains("eap_identity")
            || l.contains("authentication of '")
    ...
    if out.len() > 180 {
        ...
        compact.extend_from_slice(&out[out.len() - 120..]);
        compact.join("\n")
    } else {
        out.join("\n")
    }
...
fn engine_events_pretty() -> String {
    let Ok(raw) = fs::read_to_string(ENGINE_EVENTS) else {
        return String::new();
    };
    let mut out = Vec::new();
...
fn candidate_ips(host: &str) -> Vec<String> {
    ...
    if host == "ee-tll.prod.surfshark.com" {
        out.push("185.174.159.123".into());
    }
    for ip in bundled_endpoints::for_host(host) {
        if !out.iter().any(|x| x == ip) {
...
fn recent_health_blocks(host: &str, endpoint: &str) -> bool {
    let fresh = fs::metadata(ENDPOINT_HEALTH)
    ...
    let Ok(raw) = fs::read_to_string(ENDPOINT_HEALTH) else {
        return false;
    };
    ...
    let outcome = v
        ...
        .and_then(|x| x.get("last_outcome"))
        .and_then(Value::as_str)
        .unwrap_or("");
    matches!(
        outcome,
        "DATA_PATH_BLOCKED" | "HANDSHAKE_FAILED" | "TIMEOUT" | "POST_TUNNEL_FAILED" | "AUTH_FAILED"
    )
}
fn ike_probe_ip(target: &str, nat_t: bool) -> Result<Option<u32>, String> {
    ...
    let output = cmd
        .output()
        .map_err(|e| format!("ike-scan unavailable: {e}"))?;
...
fn ike_latency(host: &str) -> Result<Option<u32>, String> {
    ...
    for ip in candidates {
        if recent_health_blocks(host, &ip) {
            continue;
        }
...
async fn ping_location(host: String) -> Result<Option<u32>, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || ike_latency(&host))
        .await
        .map_err(|e| e.to_string())?
}
fn ping_batch_blocking(items: Vec<PingRequest>) -> Result<Vec<PingResult>, String> {
    if items.len() > 256 {
        return Err("too many ping targets".into());
    }
    if items
        .iter()
        .any(|x| x.id.len() > 64 || !valid_host(&x.host))
    {
    ...
    for chunk in items.chunks(6) {
        let handles = chunk
            ...
            .cloned()
            .map(|item| {
                thread::spawn(move || PingResult {
                    id: item.id,
                    ping: ike_latency(&item.host).ok().flatten(),
                })
            })
            .collect::<Vec<_>>();
        for h in handles {
...
async fn ping_locations_batch(items: Vec<PingRequest>) -> Result<Vec<PingResult>, String> {
    tauri::async_runtime::spawn_blocking(move || ping_batch_blocking(items))
        .await
        .map_err(|e| e.to_string())?
}
...
fn helper_output(action: &str, args: &[&str]) -> Result<String, String> {
    let limit = match action {
        "disconnect" | "cancel-connect" => "20s",
        "engine-connect" => "170s",
        "connect" | "connect-saved" | "quick-connect" => "45s",
        _ => "30s",
    };
    ...
    if output.status.success() {
        ...
        Err(format!("{action} exceeded its safety deadline ({limit}); the worker was stopped without blocking the UI.\n{text}"))
    } else {
        Err(text)
    }
}
...
fn engine_value(key: &str) -> Option<String> {
    let raw = fs::read_to_string(ENGINE_STATE).ok()?;
    let v: Value = serde_json::from_str(&raw).ok()?;
    v.get(key).and_then(|x| x.as_str()).map(str::to_string)
}
fn state_u64(path: &str, key: &str) -> u64 {
    state_value(path, key)
...
fn protected_route() -> bool {
    ...
    Command::new("ip")
        ...
        .output()
        .map(|o| o.status.success() && String::from_utf8_lossy(&o.stdout).contains(&tunnel_if))
        .unwrap_or(false)
}
fn tunnel_active() -> bool {
    ...
    if phase == "CONNECTED" {
        return protected_route()
            || engine_value("protocol").as_deref() == Some("wireguard")
            || engine_value("protocol").as_deref() == Some("openvpn");
    }
    let xfrm = fs::metadata("/sys/class/net/milmitxfrm0").is_ok();
    let live = state_value(LIVE, "STATE")
        .or_else(|| state_value(LIVE, "HEALTH"))
        .unwrap_or_default()
        .to_ascii_uppercase();
...
fn connect_location_blocking(id: String, generation: u64) -> Result<String, String> {
    let loc = parse_locations()
        .into_iter()
        .find(|x| x.id == id)
        .ok_or_else(|| "unknown location".to_string())?;
    let candidates = candidate_ips(&loc.host);
    if candidates.is_empty() {
        return Err(format!(
            "No trusted direct-IP candidate is bundled for {}. Refresh the endpoint catalog first.",
            loc.city
        ));
    }
    let csv = candidates.join(",");
    attempt_add(&format!(
        "ENGINE_V3 location={} city={} identity={} candidates={}",
        loc.id, loc.city, loc.host, csv
    ));
    ...
    match helper_output("engine-connect", &[&loc.host, &csv]) {
        Ok(text) => {
            ...
            if engine_value("phase").as_deref() == Some("CONNECTED") || tunnel_active() {
                let protocol = engine_value("protocol").unwrap_or_else(|| "ikev2".into());
                let endpoint = engine_value("endpoint").unwrap_or_default();
                attempt_add(&format!(
                    "CONNECTED protocol={} endpoint={}",
                    protocol, endpoint
                ));
                Ok(format!(
                    "LOCATION={}\nCITY={}\nIDENTITY={}\nPROTOCOL={}\nENDPOINT={}\n{}",
                ...
                Err(format!(
                    "Connection engine returned without a verified tunnel.\n{}",
                    safe
                ))
            }
        }
        Err(e) => {
            ...
            attempt_add(&format!("ENGINE_V3_FAIL\n{}", safe));
            Err(safe)
        }
    }
}
...
async fn connect_location(id: String) -> Result<String, String> {
    ...
    attempt_clear();
    tauri::async_runtime::spawn_blocking(move || connect_location_blocking(id, generation))
        .await
        .map_err(|e| e.to_string())?
}
#[tauri::command]
async fn cancel_connect() -> Result<String, String> {
    ...
    tauri::async_runtime::spawn_blocking(|| helper_output("cancel-connect", &[]))
        .await
        .map_err(|e| e.to_string())?
}
...
fn connection_state() -> ConnectionState {
    ...
    let engine_phase = engine_value("phase");
    let raw_state = engine_phase
        .or_else(|| state_value(LIVE, "STATE"))
        .or_else(|| state_value(LIVE, "HEALTH"));
    let state = if connected {
        raw_state
            .filter(|s| {
                !s.eq_ignore_ascii_case("DISCONNECTED") && !s.eq_ignore_ascii_case("UNPROTECTED")
            })
    ...
    ConnectionState {
        connected,
        state,
        public_ip: if connected {
            state_value(STATE, "PUBLIC_IP").or_else(|| state_value(LIVE, "PUBLIC_IP"))
        } else {
            None
        },
        exit_country: if connected {
            state_value(STATE, "EXIT_COUNTRY").or_else(|| state_value(LIVE, "EXIT_COUNTRY"))
        } else {
            None
        },
        latency_ms: if connected {
            state_value(LIVE, "LATENCY_MS").and_then(|v| v.parse().ok())
        } else {
            None
        },
    }
...
fn traffic_snapshot() -> TrafficSnapshot {
    let connected = tunnel_active();
    let iface = state_value(STATE, "TUNNEL_IF").unwrap_or_else(|| "milmitxfrm0".into());
    let rxp = format!("/sys/class/net/{iface}/statistics/rx_bytes");
    let txp = format!("/sys/class/net/{iface}/statistics/tx_bytes");
    TrafficSnapshot {
        connected,
        rx_bytes: if connected { read_u64(&rxp) } else { 0 },
        tx_bytes: if connected { read_u64(&txp) } else { 0 },
        rx_bps: state_u64(LIVE, "RX_BPS"),
        tx_bps: state_u64(LIVE, "TX_BPS"),
        all_rx_bytes: state_u64(USAGE, "ALL_RX_BYTES"),
        all_tx_bytes: state_u64(USAGE, "ALL_TX_BYTES"),
        day_rx_bytes: state_u64(USAGE, "DAY_RX_BYTES"),
        day_tx_bytes: state_u64(USAGE, "DAY_TX_BYTES"),
        month_rx_bytes: state_u64(USAGE, "MONTH_RX_BYTES"),
...
async fn ping_report(kind: String, host: Option<String>) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let target = match kind.as_str() {
            ...
            "location" | "vpn" => {
                let h = host.ok_or_else(|| "location hostname required".to_string())?;
                ping_target(&h)
            }
...
fn save_credentials_blocking(username: String, password: String) -> Result<String, String> {
    ...
    if output.status.success() {
        ...
        Err("Saving credentials exceeded its safety deadline.".into())
    } else {
        Err(text)
    }
}
...
async fn save_credentials(username: String, password: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || save_credentials_blocking(username, password))
        .await
        .map_err(|e| e.to_string())?
}
#[tauri::command]
async fn helper_action(action: String, args: Vec<String>) -> Result<String, String> {
    ...
    if action == "credentials-save" {
        return Err(
            "Use the secure save_credentials command so the password is sent over stdin.".into(),
        );
    }
    tauri::async_runtime::spawn_blocking(move || {
        let refs = args.iter().map(String::as_str).collect::<Vec<_>>();
        helper_output(&action, &refs)
    })
    .await
    .map_err(|e| e.to_string())?
}
#[tauri::command]
async fn router_state() -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(|| helper_json("router-status", &[]))
        .await
        .map_err(|e| e.to_string())?
}
#[tauri::command]
async fn desktop_feature_state() -> Result<Value, String> {
    tauri::async_runtime::spawn_blocking(|| helper_json("desktop-status", &[]))
        .await
        .map_err(|e| e.to_string())?
}
...
fn list_desktop_apps() -> Vec<DesktopApp> {
    ...
    for dir in desktop_dirs() {
        ...
        for ent in rd.flatten() {
            ...
            if desktop_value(&text, "NoDisplay").as_deref() == Some("true")
                || desktop_value(&text, "Hidden").as_deref() == Some("true")
            {
                continue;
            }
            ...
            let Some(exec) = desktop_value(&text, "Exec") else {
                continue;
            };
...
fn user_config_dir() -> PathBuf {
    PathBuf::from(std::env::var("HOME").unwrap_or_else(|_| ".".into()))
        .join(".config/milmit-secure")
}
fn lists_path() -> PathBuf {
    user_config_dir().join("location-lists.json")
}
#[tauri::command]
fn get_location_lists() -> Value {
    fs::read_to_string(lists_path())
        .ok()
...
fn save_location_lists(lists: Value) -> Result<(), String> {
    if !lists.is_array() {
        return Err("location lists must be an array".into());
    }
    let raw = serde_json::to_string_pretty(&lists).map_err(|e| e.to_string())?;
    if raw.len() > 131072 {
        return Err("location lists are too large".into());
    }
    let dir = user_config_dir();
    fs::create_dir_all(&dir).map_err(|e| e.to_string())?;
    fs::write(lists_path(), raw).map_err(|e| e.to_string())
}
fn autostart_path() -> PathBuf {
    PathBuf::from(std::env::var("HOME").unwrap_or_else(|_| ".".into()))
        .join(".config/autostart/milmit-secure.desktop")
}
#[tauri::command]
...
fn set_launch_at_startup(enabled: bool) -> Result<(), String> {
    ...
    let repo_root = exe
        ...
        .and_then(|p| p.parent())
        .map(PathBuf::from);
    let dev_launcher = repo_root
        .as_ref()
        .map(|r| r.join("scripts/run-tauri-gui.sh"));
    let exec_line = if let Some(script) = dev_launcher.filter(|p| p.exists()) {
        format!("/bin/bash \"{}\"", script.display())
    } else {
        format!("\"{}\"", exe.display())
    };
