...
final class OpenVPNPacketTunnelProvider: NEPacketTunnelProvider {
    ...
    override func startTunnel(options: [String: NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        guard let ovpn = options?["ovpn"] as? NSData, ovpn.length > 0, ovpn.length <= 512 * 1024 else {
        ...
        do { evaluation = try adapter.apply(configuration: configuration) }
        ...
        reachability.startTracking { [weak self] status in
            if status == .reachableViaWiFi { self?.adapter.reconnect(interval: 5) }
        }
