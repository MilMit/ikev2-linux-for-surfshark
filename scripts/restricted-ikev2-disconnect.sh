...
mkdir -p "$STATE_DIR"
# Persistent manual-disconnect marker: the watchdog must never reconnect a tunnel
# the user explicitly disconnected. A later successful connection clears it.
touch "$MANUAL_DISCONNECTED" "$DISCONNECTING"
...
remove_chains(){
  ...
  for spec in "mangle:$CHAIN_DNS" "mangle:$CHAIN_POLICY" "mangle:$CHAIN_HOST" "mangle:$CHAIN_HOT" "nat:$CHAIN_HOT_DNS" "mangle:$CHAIN_MSS" "filter:$CHAIN_BLOCK" "filter:$CHAIN_KILL" "filter:$CHAIN_HOT_FWD" "filter:$CHAIN_DEVICE_BLOCK"; do
...
# Terminate IKE first so strongSwan cannot reinstall policies while we tear down routing.
swanctl --terminate --ike "$CONN_NAME" >/dev/null 2>&1 || true
swanctl --terminate --ike surfshark-tr >/dev/null 2>&1 || true
for _ in 1 2 3 4 5; do
  swanctl --list-sas 2>/dev/null | grep -qE 'milmit-surfshark-restricted|surfshark-tr' || break
  sleep 0.2
  swanctl --terminate --ike "$CONN_NAME" >/dev/null 2>&1 || true
  swanctl --terminate --ike surfshark-tr >/dev/null 2>&1 || true
done
...
# Fail closed on the *disconnect operation*: don't claim success while our tunnel
# artifacts are still present. Normal Internet may remain available directly,
# which is expected after a normal disconnect.
left=0
...
if ((left)); then
  echo "Disconnect teardown incomplete; not reporting a false success." >&2
  exit 70
fi

echo "Restricted Surfshark IKEv2 fully disconnected. IKE SA, XFRM interface, VPN routes, hotspot VPN hooks and runtime state are gone. Direct Internet may still work normally."
