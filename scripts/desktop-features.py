...
def kv(path):
    ...
    try:
        for line in pathlib.Path(path).read_text(errors='replace').splitlines():
            if '=' in line:k,v=line.split('=',1);out[k]=v.strip()
    except OSError:pass
    return out
def load():
    ...
    return d
def save(d):VAR.mkdir(parents=True,exist_ok=True);tmp=CFG.with_suffix('.tmp');tmp.write_text(json.dumps(d,indent=2));os.chmod(tmp,0o600);tmp.replace(CFG)
...
def physical_iface():
    ...
    rc,out=run(['ip','-4','route','get','1.1.1.1','table','main']);m=re.search(r'\bdev\s+(\S+)',out);return m.group(1) if m else ''
def setup_direct_ns():
    ifc=physical_iface()
    if not ifc:raise RuntimeError('Could not determine the physical uplink for Direct app routing.')
    sysctl_forward();run(['ip','netns','add',NS])
    if not pathlib.Path('/sys/class/net',VETH_HOST).exists():run(['ip','link','add',VETH_HOST,'type','veth','peer','name',VETH_NS],check=True);run(['ip','link','set',VETH_NS,'netns',NS],check=True)
    run(['ip','addr','replace',HOST_IP,'dev',VETH_HOST]);run(['ip','link','set',VETH_HOST,'up']);run(['ip','netns','exec',NS,'ip','addr','replace',NS_IP,'dev',VETH_NS]);run(['ip','netns','exec',NS,'ip','link','set','lo','up']);run(['ip','netns','exec',NS,'ip','link','set',VETH_NS,'up']);run(['ip','netns','exec',NS,'ip','route','replace','default','via','10.254.42.1'])
...
def desktop_exec(path):
    cp=configparser.ConfigParser(interpolation=None,strict=False);cp.read(path,encoding='utf-8');sec=cp['Desktop Entry'];raw=sec.get('Exec','').strip()
...
def caller_uid():
    v=os.environ.get('PKEXEC_UID') or os.environ.get('SUDO_UID');return int(v) if v and v.isdigit() else 1000
def launch_direct(desktop_id):
    uid=caller_uid();path=find_desktop(desktop_id,uid);args,name=desktop_exec(path);setup_direct_ns();import pwd;pw=pwd.getpwuid(uid);runtime=f'/run/user/{uid}';env=['HOME='+pw.pw_dir,'USER='+pw.pw_name,'LOGNAME='+pw.pw_name,'XDG_RUNTIME_DIR='+runtime,'DBUS_SESSION_BUS_ADDRESS=unix:path='+runtime+'/bus']
    if pathlib.Path(runtime+'/wayland-0').exists():env.append('WAYLAND_DISPLAY=wayland-0')
    env.append('DISPLAY=:0');cmd=['ip','netns','exec',NS,'runuser','-u',pw.pw_name,'--','env',*env,*args];subprocess.Popen(cmd,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,start_new_session=True);return {'ok':True,'name':name,'desktop_id':desktop_id,'mode':'direct'}
