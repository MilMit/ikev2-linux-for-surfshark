...
needs_install=0
if [[ ! -x "$HELPER" || ! -f "$INSTALLED_CONNECT" || ! -x "$INSTALLED_CONNECT_V2" || ! -f "$INSTALLED_DISCONNECT" || ! -x "$INSTALLED_DEVICE_POLICY" || ! -x "$INSTALLED_DEVICE_MANAGER" || ! -x "$INSTALLED_WATCHDOG" || ! -x "$INSTALLED_CONTROL" || ! -f "$WATCHDOG_UNIT" ]]; then
  needs_install=1
...
if [[ "$needs_install" == 1 ]]; then
  echo "VPN backend/control center/watchdog update: Ubuntu may ask for your password once."
...
if [[ ! -x "$HELPER" ]] \
  ...
  || ! cmp -s "$ROOT/scripts/control-center.py" "$INSTALLED_CONTROL" \
  || ! cmp -s "$ROOT/scripts/milmit-surfshark-helper" "$HELPER"; then
  echo "ERROR: privileged VPN backend/control center is stale or installation did not complete." >&2
  exit 78
fi
...
echo "MilMit VPN backend, protection control center, watchdog and hotspot manager: verified and current."
...
if [[ "${1:-}" == "bash" && -n "${2:-}" ]]; then
  ...
  case "$script" in
    */scripts/restricted-ikev2-connect.sh)
      endpoint="${3:-}"; username="${4:-}"; mss="${5:-1200}"; dns_csv="${6:-162.252.172.57,149.154.159.92}"
      hotspot_vpn="${7:-1}"; recover_network="${8:-1}"; hotspot_iface="${9:-auto}"; kill_switch="${10:-0}"
      routing_mode="${11:-vpn_all}"; hotspot_vpn_macs="${12:-}"; hotspot_direct_macs="${13:-}"
