...
while true; do
  ...
  # Every ~30 seconds: refresh device policies/quotas and repair hotspot hooks if NM recreated the AP.
  ...
  if [[ "$sa_ok" == 1 && "$route_ok" == 1 && -n "$vip" ]]; then failures=0; write_live OK "$rx_bps" "$tx_bps" "$latency" 0 protected
    ...
    if ((failures>=3)) && [[ ! -e "$DISCONNECTING" ]]; then write_live RECOVERING "$rx_bps" "$tx_bps" "$latency" "$failures" auto-reconnect; if quick_reconnect; then failures=0; else sleep 5; fi; fi
  fi
done
