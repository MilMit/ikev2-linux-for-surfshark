...
if [[ -n "$PHYS_IFACE" ]]; then
  iptables -w -t filter -A "$CHAIN_HOT_FWD" -i "$HOTSPOT_IFACE" -s "$HOTSPOT_SUBNET" -o "$PHYS_IFACE" -m mark --mark "$MARK_DIRECT" -j ACCEPT
  iptables -w -t filter -A "$CHAIN_HOT_FWD" -i "$PHYS_IFACE" -o "$HOTSPOT_IFACE" -d "$HOTSPOT_SUBNET" -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
else
