...
CFG=VAR/'router-features.json'; EVENTS=VAR/'events.jsonl'; USAGE=VAR/'device-usage.json'; GUEST=VAR/'guest-hotspot.json'
POLICY_CHAIN='MILMIT_ADV_POLICY'; BLOCK_CHAIN='MILMIT_ADV_BLOCK'; DEV_BLOCK_CHAIN='MILMIT_DEVICE_BLOCK'; DEV_USAGE_CHAIN='MILMIT_DEVICE_USAGE'
FORCE_DIRECT='MILMIT_FORCE_DIRECT'; FORCE_VPN='MILMIT_FORCE_VPN'; BLOCK_SET='MILMIT_BLOCK'
MARK_VPN='0x112'; MARK_DIRECT='0x113'; XFRM='milmitxfrm0'

def run(args, timeout=12):
    try:
        p=subprocess.run(args,text=True,capture_output=True,timeout=timeout); return p.returncode,(p.stdout+('\n' if p.stdout and p.stderr else '')+p.stderr).strip()
...
def save_json(path,obj,mode=0o600):
    path.parent.mkdir(parents=True,exist_ok=True);tmp=path.with_suffix(path.suffix+'.tmp');tmp.write_text(json.dumps(obj,ensure_ascii=False,indent=2));os.chmod(tmp,mode);tmp.replace(path)
def event(kind,msg,**extra):
    VAR.mkdir(parents=True,exist_ok=True);row={'time':datetime.now(timezone.utc).isoformat(),'kind':kind,'message':msg,**extra}
    with EVENTS.open('a') as f:f.write(json.dumps(row,ensure_ascii=False)+'\n')
    os.chmod(EVENTS,0o600)
def defaults():return {'force_dns':True,'block_quic':False,'client_isolation':False,'ipv6_policy':'block','devices':{},'policies':[],'guest':{'duration_minutes':60,'speed_kbit':0},'priority':['block','force_vpn','manual_direct','iran_direct','default']}
def cfg():
    d=defaults();d.update(load_json(CFG,{}));return d
...
def unhook(table,base,chain):
    for _ in range(20):
        ...
        if rc:break
def chain(table,name):ipt(table,'-N',name);ipt(table,'-F',name)
def resolve_target(target):
    target=target.strip().lower();vals=[]
    try:vals=[str(ipaddress.ip_network(target,strict=False))]
    except ValueError:
        host=re.sub(r'^https?://','',target).split('/')[0].split(':')[0];rc,text=run(['getent','ahostsv4',host],6)
        if rc==0:
            for line in text.splitlines():
                ip=line.split()[0]
                try:ipaddress.ip_address(ip);vals.append(ip+'/32')
                except ValueError:pass
    return sorted(set(vals))
def hotspot_info():
    st=kv(STATE);iface=st.get('HOTSPOT_IFACE','');subnet=st.get('HOTSPOT_SUBNET','');dns=st.get('HOTSPOT_DNS','');clients=[]
    if iface:
...
def add_policy(target,policy,scope='both'):
    ...
    c=cfg();rows=[x for x in c['policies'] if not(x.get('target')==target and x.get('scope')==scope)];rows.append({'target':target,'policy':policy,'scope':scope,'resolved':resolve_target(target),'updated':int(time.time())});c['policies']=rows;save_json(CFG,c);apply();event('policy',f'{target} -> {policy}',scope=scope);return {'ok':True,'policy':rows[-1]}
...
def read_usage_counters():
    counters={};rc,text=ipt('filter','-L',DEV_USAGE_CHAIN,'-vnx')
    if rc:return counters
    for line in text.splitlines():
        m=re.search(r'^\s*\d+\s+(\d+).*?/\*\s+(UP|DOWN):([0-9A-F:]{17})\s+\*/',line,re.I)
        if m:counters.setdefault(m.group(3).upper(),{})[m.group(2).lower()]=int(m.group(1))
    return counters

def update_usage(clients):
    today=time.strftime('%Y-%m-%d');u=load_json(USAGE,{'day':today,'devices':{}});day_changed=u.get('day')!=today
    if day_changed:u['day']=today
    counters=read_usage_counters();now=int(time.time())
    for cl in clients:
        mac=cl['mac'];cur=counters.get(mac,{});row=u.setdefault('devices',{}).setdefault(mac,{})
        if day_changed:row['day_up_bytes']=0;row['day_down_bytes']=0
        for direction in ('up','down'):
            raw=int(cur.get(direction,0));last=int(row.get('raw_'+direction,0));delta=raw-last if raw>=last else raw
            row[direction+'_bytes']=int(row.get(direction+'_bytes',0))+max(0,delta);row['day_'+direction+'_bytes']=int(row.get('day_'+direction+'_bytes',0))+max(0,delta);row['raw_'+direction]=raw
        row['bytes_seen']=int(row.get('day_up_bytes',0))+int(row.get('day_down_bytes',0));row['updated']=now;row['ip']=cl.get('ip','')
    save_json(USAGE,u);return u

def setup_usage_chain(clients):
    chain('filter',DEV_USAGE_CHAIN)
    for cl in clients:
        ip=cl.get('ip');mac=cl.get('mac')
        if not ip or not mac:continue
        ipt('filter','-A',DEV_USAGE_CHAIN,'-s',ip+'/32','-m','comment','--comment','UP:'+mac,'-j','RETURN')
        ipt('filter','-A',DEV_USAGE_CHAIN,'-d',ip+'/32','-m','comment','--comment','DOWN:'+mac,'-j','RETURN')
    unhook('filter','FORWARD',DEV_USAGE_CHAIN);ipt('filter','-I','FORWARD','1','-j',DEV_USAGE_CHAIN)

def apply_tc(iface,clients,c):
    if not iface:return
    run(['tc','qdisc','del','dev',iface,'root'],4);limited=[]
    for cl in clients:
        d=c['devices'].get(cl['mac'],{});rate=int(d.get('speed_kbit',0) or 0)
        if rate>0:limited.append((cl['ip'],rate))
    if not limited:return
    run(['tc','qdisc','add','dev',iface,'root','handle','1:','htb','default','999'],5);run(['tc','class','add','dev',iface,'parent','1:','classid','1:999','htb','rate','1000mbit','ceil','1000mbit'],5)
    for idx,(ip,rate) in enumerate(limited,10):run(['tc','class','add','dev',iface,'parent','1:','classid',f'1:{idx}','htb','rate',f'{rate}kbit','ceil',f'{rate}kbit'],5);run(['tc','filter','add','dev',iface,'protocol','ip','parent','1:','prio',str(idx),'u32','match','ip','dst',ip+'/32','flowid',f'1:{idx}'],5)
def apply():
    ...
    for p in c['policies']:
        resolved=p.get('resolved') or resolve_target(p.get('target',''));p['resolved']=resolved;setname={'direct':FORCE_DIRECT,'vpn':FORCE_VPN,'block':BLOCK_SET}.get(p.get('policy'))
        if setname:
            for net in resolved:run(['ipset','add',setname,net,'-exist'])
    save_json(CFG,c);chain('mangle',POLICY_CHAIN);ipt('mangle','-A',POLICY_CHAIN,'-m','set','--match-set',FORCE_VPN,'dst','-j','MARK','--set-mark',MARK_VPN);ipt('mangle','-A',POLICY_CHAIN,'-m','set','--match-set',FORCE_VPN,'dst','-j','RETURN');ipt('mangle','-A',POLICY_CHAIN,'-m','set','--match-set',FORCE_DIRECT,'dst','-j','MARK','--set-mark',MARK_DIRECT);ipt('mangle','-A',POLICY_CHAIN,'-m','set','--match-set',FORCE_DIRECT,'dst','-j','RETURN');unhook('mangle','OUTPUT',POLICY_CHAIN);ipt('mangle','-I','OUTPUT','1','-j',POLICY_CHAIN)
    if iface and subnet:unhook('mangle','PREROUTING',POLICY_CHAIN);ipt('mangle','-I','PREROUTING','1','-i',iface,'-s',subnet,'-j',POLICY_CHAIN)
    chain('filter',BLOCK_CHAIN);ipt('filter','-A',BLOCK_CHAIN,'-m','set','--match-set',BLOCK_SET,'dst','-j','REJECT','--reject-with','icmp-net-unreachable')
    if c.get('block_quic'):ipt('filter','-A',BLOCK_CHAIN,'-p','udp','--dport','443','-j','REJECT','--reject-with','icmp-port-unreachable')
    unhook('filter','OUTPUT',BLOCK_CHAIN);ipt('filter','-I','OUTPUT','1','-j',BLOCK_CHAIN);unhook('filter','FORWARD',BLOCK_CHAIN);ipt('filter','-I','FORWARD','1','-j',BLOCK_CHAIN)
    ...
    unhook('filter','FORWARD',DEV_BLOCK_CHAIN);ipt('filter','-I','FORWARD','1','-j',DEV_BLOCK_CHAIN);setup_usage_chain(hs['clients'])
    vpn=[];direct=[]
    ...
    if STATE.exists() and iface and subnet:
        ...
        if c.get('client_isolation'):ipt('filter','-I',DEV_BLOCK_CHAIN,'1','-i',iface,'-o',iface,'-j','DROP')
        run(['sysctl','-q','-w',f'net.ipv6.conf.{iface}.disable_ipv6='+('1' if c.get('ipv6_policy')=='block' else '0')],4);apply_tc(iface,hs['clients'],c)
    event('apply','Advanced router policies applied',hotspot=iface,clients=len(hs['clients']));return {'ok':True,'hotspot':hs,'options':{k:c[k] for k in ('force_dns','block_quic','client_isolation','ipv6_policy')},'devices':c['devices'],'policies':c['policies'],'usage':usage}
def quota_enforce():
...
def guest_start(minutes=60,ssid='MilMit Guest'):
    ...
    st=kv(STATE);avoid={st.get('IFACE',''),st.get('HOTSPOT_IFACE','')};rc,text=run(['nmcli','-t','-f','DEVICE,TYPE,STATE','device','status'],6);dev=''
    if rc==0:
        for line in text.splitlines():
            p=line.split(':')
    ...
    password=''.join(secrets.choice(string.ascii_letters+string.digits) for _ in range(14));con='MilMit Guest Hotspot';run(['nmcli','connection','delete',con],5);rc,out=run(['nmcli','device','wifi','hotspot','ifname',dev,'con-name',con,'ssid',ssid,'password',password],20)
    if rc!=0:return {'ok':False,'error':out}
    run(['nmcli','connection','modify',con,'connection.autoconnect','no','ipv4.method','shared','ipv6.method','disabled'],8);expires=int(time.time()+minutes*60);data={'iface':dev,'connection':con,'ssid':ssid,'password':password,'expires':expires,'minutes':minutes,'wifi_uri':f'WIFI:T:WPA;S:{ssid};P:{password};;'};save_json(GUEST,data);run(['systemd-run','--unit=milmit-guest-expire','--on-active',f'{minutes}m','/usr/libexec/milmit-surfshark-helper','guest-stop'],8);event('guest-start','Guest hotspot started',iface=dev,expires=expires);return {'ok':True,**data}
def guest_stop():
    g=load_json(GUEST,{})
...
def status():
    hs=hotspot_info();usage=update_usage(hs['clients']);return {'ok':True,'hotspot':hs,'config':cfg(),'guest':guest_status(),'usage':usage}
def main():
    if os.geteuid()!=0:print(json.dumps({'ok':False,'error':'router feature engine must run as root'}));return 77
    VAR.mkdir(parents=True,exist_ok=True);cmd=sys.argv[1] if len(sys.argv)>1 else 'status'
    try:
