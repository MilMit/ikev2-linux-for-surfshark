...
impl LinuxPlatformAdapter {
    ...
    fn save_credentials_root(&self, username: &str, password: &str) -> Result<(), String> {
        ...
        let mut child = Command::new("pkexec")
            ...
            .stderr(Stdio::piped())
            .spawn()
            .map_err(|e| format!("failed to launch credential helper: {e}"))?;
    ...
    fn spawn_forced_protocol(&self, protocol: &str, identity: &str) -> Result<(), String> {
        if !Path::new(PROTOCOL_CONNECTOR).is_file() {
            return Err(format!("forced protocol connector is not installed at {PROTOCOL_CONNECTOR}"));
        }
    ...
    fn probe_target(&self, target: &str) -> Result<Option<u32>, String> {
        if !Path::new("/usr/bin/ike-scan").is_file() && !Path::new("/usr/sbin/ike-scan").is_file() {
            return Ok(None);
        }
        let started = Instant::now();
        let output = Command::new("ike-scan")
            .args(["--ikev2", "--sport=0", "--retry=1", "--timeout=1200", "--nodns", "--quiet", target])
            .output()
...
impl PlatformAdapter for LinuxPlatformAdapter {
    fn connect(&self, location: &Location, protocol: VpnProtocol) -> Result<PlatformStatus, String> {
        ...
        match protocol {
            VpnProtocol::Auto | VpnProtocol::Ikev2 => {
                ...
                self.spawn_engine_connect(identity.to_string(), candidates)?;
            }
            VpnProtocol::WireGuard => {
                if !Self::profile_exists(WG_DIR, identity, "conf") { return Err(format!("WireGuard profile is missing for {identity}")); }
                self.spawn_forced_protocol("wireguard", identity)?;
            }
            VpnProtocol::OpenVpn => {
                if !Self::profile_exists(OVPN_DIR, identity, "ovpn") { return Err(format!("OpenVPN profile is missing for {identity}")); }
                self.spawn_forced_protocol("openvpn", identity)?;
            }
        }
        Ok(PlatformStatus {
            status: PlatformConnectionStatus::Connecting,
            protocol: Some(match protocol { VpnProtocol::Auto => "auto", VpnProtocol::Ikev2 => "ikev2", VpnProtocol::WireGuard => "wireguard", VpnProtocol::OpenVpn => "openvpn" }.into()),
            endpoint: None,
            public_ip: None,
            message: Some("Connection worker started".into()),
        })
    }
    ...
    fn credentials_status(&self) -> Result<bool, String> {
        if !Path::new(HELPER).is_file() { return Ok(false); }
        let text = self.helper("credentials-status", &[], "15s")?;
        Ok(text.lines().any(|line| line.trim() == "SAVED=1"))
    }

    fn save_credentials(&self, username: &str, password: &str) -> Result<(), String> {
        self.save_credentials_root(username, password)
    }

    fn probe_latency(&self, location: &Location) -> Result<Option<u32>, String> {
        let candidates = self.candidates(location)?;
        let mut best = None;
        for target in candidates.into_iter().take(6) {
            if let Some(ms) = self.probe_target(&target)? {
                best = Some(best.map_or(ms, |current: u32| current.min(ms)));
            }
        }
        Ok(best)
    }
    ...
    fn diagnostics(&self) -> Result<PlatformDiagnostics, String> {
        ...
        Ok(PlatformDiagnostics {
            ...
            details: serde_json::json!({
                ...
                "helper": HELPER,
                "forced_protocol_connector": PROTOCOL_CONNECTOR,
